...
case "$choice" in
    ...
    2)
        ...
        echo -e "  ${GREEN}${BOLD}Ready.${NC}"
        echo ""
        echo -e "  ${CYAN}Start SafeClaw (container — recommended):${NC}"
        echo ""
        echo "    docker run -p 8899:8899 -v ~/safeclaw:/workspace ghcr.io/aceteam-ai/aep-proxy"
        echo ""
        echo -e "  ${CYAN}Start SafeClaw (pip — developer mode):${NC}"
        echo ""
